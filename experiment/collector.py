...
class ExperimentCollector(Listener):
    ...
    def receive(self, event, modifier, data : dict):
        if event == settings.TRAIN_ONE_EPOCH_COMPLETE:
            ...
            hparams_domain = {
                'rate' : [0.95, 0.8, 0.65, 0.5, 0.35, 0.2, 0.05, 0.0],
                'test_rate' : [0.95, 0.8, 0.65, 0.5, 0.35, 0.2, 0.05, 0.0],
                'trigger' : ['checker', 'white'],
                'position' : ['default', 'random']
            }
            
            self.writer.add_hparams(hparams, {'best' : metric, 'bd_metric': bd_metric}, hparams_domain)
